...
function toParsedDocument(document: Document): ParsedDocument {
  const parsedContent = parseReadingContent(document.content);
...
export default function ReadPageClient({
  ...
  const [document, setDocument] = useState<ParsedDocument | null>(() => toParsedDocument(initialDocument));
  ...
  useEffect(() => {
    const load = async () => {
      setDocument(toParsedDocument(initialDocument));
      setReadCountCooldownMs(getReadCountCooldownMsForContent(initialDocument.content));

      try {
        const [settingsRes, progressRes] = await Promise.all([
          fetch(`/api/settings`),
          fetch(`/api/reading-progress?documentId=${documentId}`),
        ]);
  ...
  }, [csrfFetch, documentId, readCountCooldownMs]);
  ...
  }, [csrfFetch, document, documentId, readingFocus.currentLine]);
  ...
  }, [csrfFetch, documentId, sessionId]);
    ...
    <div className="flex flex-col gap-4 pb-24">
      ...
      <div className="flex items-center gap-4">
        ...
        <span
          ...
          aria-label={`${reading("settings.theme")}: ${themeOptions.find((themeOption) => themeOption.value === readingFocus.theme)?.label ?? ""}`}
          title={themeOptions.find((themeOption) => themeOption.value === readingFocus.theme)?.label}
          aria-live="polite"
        >
          {themeOptions.find((themeOption) => themeOption.value === readingFocus.theme)?.icon ?? "🔦"}
        </span>
      </div>
